...
class SecurityUtil
{
     ...
     * @param array<string, mixed> $options The options for password_hash
     *
     * @return string The hashed password
     */
    public function generateHash(string $password, array $options = []): string
    {
        // generate hash
     ...
     */
    public function encryptAes(string $plainText, string $key, string $method = 'AES-128-CBC'): string
    {
     ...
     */
    public function decryptAes(string $encryptedData, string $key, string $method = 'AES-128-CBC'): ?string
    {
        // derive a fixed-size key using PBKDF2 with SHA-256
        $derivedKey = hash_pbkdf2("sha256", $key, "", 10000, 32);
